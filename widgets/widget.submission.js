(function () {
    widget = Retina.Widget.extend({
        about: {
                title: "Amplicon Submission Widget",
                name: "submission",
	...
	var widget = Retina.WidgetInstances.submission[1];
	var url = RetinaConfig.mgrast_api + "/inbox";
	if (stm.user) {
	    jQuery.ajax(url, {
		success: function(data){
		    Retina.WidgetInstances.submission[1].inboxData = data.files;
		    Retina.WidgetInstances.submission[1].display();
		},
		error: function(jqXHR, error){
		    Retina.WidgetInstances.submission[1].showError("unable to get inbox data");
		    console.log(error);
		    console.log(jqXHR);
		},
		crossDomain: true,
	...
	Retina.WidgetInstances.submission[1].main.innerHTML = "<div class='alert alert-error' style='width: 500px;'>"+error+"</div>";
    };
	...
	html += "<div class='input-append' style='margin-left: 20%; margin-top: 15px; margin-bottom: 15px;'><input type='text' name='projectname' placeholder='enter project name' autocomplete='off' id='projectname' style='width: 400px;'><button class='btn' onclick='Retina.WidgetInstances.submission[1].selectProject();'>select</button></div>";
	...
	html += "<div id='fileSelect'><select name='inputfiles' id='inputfiles' multiple style='margin-left: 50px; width: 500px; height: 200px;'>"+seq_file_opts+"</select><button class='btn' onclick='Retina.WidgetInstances.submission[1].selectFiles();' style='margin-left: 20px;'>select</button></div>";

	html += "<div id='selectedFiles' style='display: none;'><div></div><button class='btn' onclick='Retina.WidgetInstances.submission[1].unselectFiles();'>unselect</button></div>";
	...
	html += "<button class='btn' onclick='Retina.WidgetInstances.submission[1].selectOptions();'>select</button>";
...
<div class="control-group"><label class="control-label" for="submitEBI"><b>submit to EBI</b></label><div class="controls"><label class="checkbox"><input id="submitEBI" name="submitEBI" value="submitEBI" type="checkbox" checked>In addition to submitting this project to MG-RAST, also submit to the <a target="blank" href="http://www.ebi.ac.uk/submission/">EBI archive</a>.</label></div>';
	...
	html += '\
<div style="height:10px;" class="clear"></div>\
<div style="margin-bottom: 20px; text-align: center;"><img src="Retina/images/waiting.gif" style="display: none; width: 24px;" id="submit_job_waiting"><input type="button" id="submit_job_button" disabled="" onclick="Retina.WidgetInstances.submission[1].submit_job();" value="submit job" class="btn"><span style="margin-left: 20px;"><b>Note: You must complete all previous steps to enable submission.</b></span></div>\
